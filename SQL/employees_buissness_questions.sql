...
# Finding the lowest department number encountered the 'dept_emp' table. Then, finding the highest department number.
SELECT
    MIN(dept_no)
FROM
    dept_emp;
...
# Obtaining a table containing the following three fields for all individuals whose employee number is not greater than 10040:
# - employee number
...
# - assign '110022' as 'manager' to all individuals whose employee number is less than or equal to 10020, and '110039' to those whose number is between 10021 and 10040.
SELECT
    emp_no,
    (SELECT
            MIN(dept_no)
...
# Retrieving a list with all employees from the ‘titles’ table who are senior engineers.
SELECT
    *
FROM
    titles
...
# Creating a procedure to insert an employee number to obtain an output containing the same number, as well as the number and name of the last department the employee has worked for.
# Calling the procedure for employee number 10010.
...
# Finding out how many contracts have been registered in the ‘salaries’ table with duration of more than one year and of value higher than or equal to $100,000.
...
# Defining a function that retrieves the largest and lowest contract salary value of an employee.
...
# Based on the previous example, creating a function that also accepts a second parameter which would be a character sequence. 
# Evaluating if its value is 'min' or 'max' and based on that retrieving either the lowest or the highest salary.
